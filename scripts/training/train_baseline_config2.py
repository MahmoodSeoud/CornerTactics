#!/usr/bin/env python3
"""
Config 2: Weighted BCE Loss for class imbalance
Hypothesis: Weighting positive class higher prevents collapse to all negatives
"""
...
if __name__ == "__main__":
    # Override with weighted BCE loss
    import scripts.training.train_baseline as tb
    import src.models.baselines as baselines

    # Create custom training function with weighted loss
    original_train_mlp_baseline = baselines.train_mlp_baseline

    def weighted_train_mlp_baseline(model, train_loader, val_loader,
                                   num_steps=10000, lr=1e-3, weight_decay=1e-4,
                                   device='cuda', eval_every=500, verbose=True,
                                   dual_task=True, shot_weight=1.0):
        """Train with weighted BCE loss for shot prediction"""
        import numpy as np
        ...
        # WEIGHTED BCE LOSS (positive class weight = 2.6)
        pos_weight = torch.tensor([2.6]).to(device)
        shot_criterion = nn.BCEWithLogitsLoss(pos_weight=pos_weight)

        history = {'train_loss': [], 'val_top1': [], 'val_top3': [], 'val_top5': [],
                  'val_shot_f1': [], 'val_shot_auroc': [], 'steps': []}
        ...
        if verbose:
            print(f"\nTraining MLP with WEIGHTED BCE LOSS")
            print(f"Positive class weight: 2.6 (to handle 27.7% positive rate)")
        ...
        while step < num_steps:
            ...
            h2 = model.dropout(h2)
            shot_logits = model.fc3_shot(h2)  # Raw logits
            ...
            if step % eval_every == 0 or step == num_steps:
                ...
                # Evaluate shot with proper sigmoid
                all_shot_preds = []
                all_shot_labels = []
                model.eval()
                with torch.no_grad():
        ...
        return {'best_val_top3': best_val_top3}

    baselines.train_mlp_baseline = weighted_train_mlp_baseline
    ...
    def custom_train_mlp(train_loader, val_loader, test_loader, device='cuda'):
        """Custom MLP training with weighted BCE"""
        print("\n" + "="*80)
        print("CONFIG 2: WEIGHTED BCE LOSS")
        print("="*80)
        ...
        print(f"  Loss: BCEWithLogitsLoss(pos_weight=2.6)")

        # Train with weighted loss
        history = weighted_train_mlp_baseline(
            model=model,
        ...
        return {
            'model': 'MLP-Config2',
            'config': 'weighted_BCE_pos_weight=2.6',
            'receiver': {
