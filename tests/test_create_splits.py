"""
Tests for Task 4: Train/Val/Test Split Creation

Tests the creation of match-based stratified train/val/test splits.
"""
...
@pytest.fixture
def val_indices_path():
    """Path to the validation indices CSV file"""
    return Path("data/processed/val_indices.csv")
...
class TestMatchBasedSplit:
    ...
    def test_split_files_exist(self, train_indices_path, val_indices_path, test_indices_path):
        """Test that all split index files are created"""
        assert train_indices_path.exists(), f"Train indices not found at {train_indices_path}"
        assert val_indices_path.exists(), f"Val indices not found at {val_indices_path}"
        assert test_indices_path.exists(), f"Test indices not found at {test_indices_path}"

    def test_no_match_overlap_train_val(self, features_csv_path, train_indices_path, val_indices_path):
        """Test that no match appears in both train and val sets"""
        df = pd.read_csv(features_csv_path)
        train_indices = pd.read_csv(train_indices_path)
        val_indices = pd.read_csv(val_indices_path)

        train_matches = set(df.iloc[train_indices['index']]['match_id'].unique())
        val_matches = set(df.iloc[val_indices['index']]['match_id'].unique())

        overlap = train_matches & val_matches
        assert len(overlap) == 0, f"Found {len(overlap)} matches in both train and val"

    def test_no_match_overlap_train_test(self, features_csv_path, train_indices_path, test_indices_path):
    ...
    def test_no_match_overlap_val_test(self, features_csv_path, val_indices_path, test_indices_path):
        """Test that no match appears in both val and test sets"""
        df = pd.read_csv(features_csv_path)
        val_indices = pd.read_csv(val_indices_path)
        test_indices = pd.read_csv(test_indices_path)
        ...
        overlap = val_matches & test_matches
        assert len(overlap) == 0, f"Found {len(overlap)} matches in both val and test"

    def test_all_corners_assigned_to_splits(self, features_csv_path, train_indices_path, val_indices_path, test_indices_path):
        """Test that all corners are assigned to train, val, or test"""
        df = pd.read_csv(features_csv_path)
        train_indices = pd.read_csv(train_indices_path)
        val_indices = pd.read_csv(val_indices_path)
        test_indices = pd.read_csv(test_indices_path)

        total_in_splits = len(train_indices) + len(val_indices) + len(test_indices)
        assert total_in_splits == len(df), f"Expected {len(df)} corners in splits, got {total_in_splits}"

    def test_split_ratio_approximately_60_20_20(self, train_indices_path, val_indices_path, test_indices_path):
        """Test that split ratio is approximately 60/20/20"""
        train_indices = pd.read_csv(train_indices_path)
        val_indices = pd.read_csv(val_indices_path)
        test_indices = pd.read_csv(test_indices_path)
        ...
        val_ratio = len(val_indices) / total
        test_ratio = len(test_indices) / total

        # Allow ±10% tolerance due to match-based grouping
        assert 0.50 <= train_ratio <= 0.70, f"Train ratio {train_ratio:.2%} not within 50-70%"
        assert 0.10 <= val_ratio <= 0.30, f"Val ratio {val_ratio:.2%} not within 10-30%"
        assert 0.10 <= test_ratio <= 0.30, f"Test ratio {test_ratio:.2%} not within 10-30%"
...
class TestStratification:
    ...
    def test_val_class_distribution_matches_overall(self, features_csv_path, val_indices_path):
        """Test that val set has similar class distribution to overall"""
        df = pd.read_csv(features_csv_path)
        val_indices = pd.read_csv(val_indices_path)

        overall_dist = df['outcome'].value_counts(normalize=True)
        val_dist = df.iloc[val_indices['index']]['outcome'].value_counts(normalize=True)

        # Check each class is within ±5% of overall distribution
        for outcome_class in overall_dist.index:
            overall_pct = overall_dist[outcome_class]
            val_pct = val_dist.get(outcome_class, 0)

            diff = abs(overall_pct - val_pct)
            assert diff <= 0.05, f"{outcome_class}: val {val_pct:.2%} differs from overall {overall_pct:.2%} by {diff:.2%}"
    ...
    def test_all_classes_present_in_val(self, features_csv_path, val_indices_path):
        """Test that all outcome classes are present in val set"""
        df = pd.read_csv(features_csv_path)
        val_indices = pd.read_csv(val_indices_path)

        overall_classes = set(df['outcome'].unique())
        val_classes = set(df.iloc[val_indices['index']]['outcome'].unique())

        missing_classes = overall_classes - val_classes
        assert len(missing_classes) == 0, f"Missing classes in val: {missing_classes}"
...
class TestOutputFormat:
    ...
    def test_val_indices_has_index_column(self, val_indices_path):
        """Test that val indices CSV has 'index' column"""
        val_indices = pd.read_csv(val_indices_path)
        assert 'index' in val_indices.columns, "Val indices missing 'index' column"
    ...
    def test_val_indices_are_valid(self, features_csv_path, val_indices_path):
        """Test that val indices are valid row numbers"""
        df = pd.read_csv(features_csv_path)
        val_indices = pd.read_csv(val_indices_path)

        max_index = val_indices['index'].max()
        assert max_index < len(df), f"Val index {max_index} out of bounds for {len(df)} rows"

        min_index = val_indices['index'].min()
        assert min_index >= 0, f"Val index {min_index} is negative"
